...
public:
  virtual int init() = 0;
  void linkDriver(BLDCDriver *_driver)
  {
    driver = _driver;
  }
  ...
  virtual float getDCCurrent(float motor_electrical_angle)
  {
...
PIDController::PIDController(float P, float I, float D, float ramp, float limit)
    : P(P), I(I), D(D), output_ramp(ramp), limit(limit), error_prev(0.0f), output_prev(0.0f), integral_prev(0.0f)
{
...
void FOCMotor::monitor()
{
  ...
  {
    if (!printed && monitor_start_char)
      monitor_port->print(monitor_start_char);
  ...
  if (monitor_variables & _MON_VOLT_Q)
  {
    if (!printed && monitor_start_char)
      ...
      monitor_port->print(monitor_separator);
  ...
  {
    if (monitor_end_char)
      monitor_port->println(monitor_end_char);
    else
...
int InlineCurrentSense::init()
{
  ...
  if (params == SIMPLEFOC_CURRENT_SENSE_INIT_FAILED)
    return 0;
  calibrateOffsets();
...
void InlineCurrentSense::calibrateOffsets()
{
  ...
  for (int i = 0; i < calibration_rounds; i++)
  {
    ...
    if (_isset(pinC))
      offset_ic += _readADCVoltageInline(pinC, params);
    _delay(1);
  }
  if (_isset(pinA))
...
void Commander::motor(FOCMotor *motor, char *user_command)
{
  ...
  case CMD_C_Q_PID:
    ...
    if (sub_cmd == SCMD_LPF_TF)
      lpf(&motor->LPF_current_q, &user_command[1]);
    else
      pid(&motor->PID_current_q, &user_command[1]);
    break;
  case CMD_C_D_PID:
    printVerbose(F("PID curr d| "));
    if (sub_cmd == SCMD_LPF_TF)
      lpf(&motor->LPF_current_d, &user_command[1]);
    else
      pid(&motor->PID_current_d, &user_command[1]);
    break;
  case CMD_V_PID:
  ...
  case CMD_INDUCTANCE:
    ...
    if (_isset(motor->phase_inductance))
      println(motor->phase_inductance);
    else
      println(0);
    break;
  case CMD_KV_RATING:
  ...
  case CMD_MONITOR:
    ...
    case SCMD_SET:
      if (!GET)
      {
        ...
        for (int i = 0; i < 7; i++)
        {
          if (isSentinel(user_command[value_index + i]))
            break;
...
void Commander::motion(FOCMotor *motor, char *user_cmd, char *separator)
{
  ...
  case CMD_STATUS:
    ...
    if (!GET)
      (bool)value ? motor->enable() : motor->disable();
...
{
  if (verbose == VerboseMode::machine_readable)
    print(number);
}
